...
@Plugin(id = "cmdcontrol", name = "CmdControl", version = "1.1.0", url = "https://ore.spongepowered.org/Simon_Flash/CmdControl", authors = "Simon_Flash")
public class CmdControl extends CmdPlugin {
    ...
    public void onInit(GameInitializationEvent event) {
        ...
        CmdPlugin.getRegistry().values().forEach(p -> {
            ...
            getLogger().info("|       - " + message + Strings.repeat(" ", 28 - message.length()) + "|");
        });
        getLogger().info("+=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=+");
